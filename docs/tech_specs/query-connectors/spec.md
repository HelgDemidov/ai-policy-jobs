...
Ветка: не создаётся — в репо ещё нет ни одного коммита, коммиты отложены решением куратора; спек-процедура адаптирована из G2AI `/tech-spec` под standalone-репо (нет CI, нет coverage-гейта, docs/ трекается).
...
Решения куратора (2026-07-25): LinkedIn — НЕ в дефолтном прогоне, только по явному флагу; тир — авто-эвристика (см. §4).
