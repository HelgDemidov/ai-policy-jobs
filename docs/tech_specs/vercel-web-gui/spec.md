...
Статус: реализовано (`9da499b`..`55b2c44`) · 2026-08-04
...
1. `feat(blob): scripts/blob_sync.py — download/upload jobs.db через Vercel CLI, врезка в run.py __main__` + тесты — `9da499b`, фикс флагов `c805e56`
2. `feat(web): web/api/ — postings/status Python-функции поверх store.py` + тесты логики — `ec59880` (без импорта `store.py` — см. «Что разошлось с планом»)
3. `feat(web): web/public/ — статическая карточная сетка, фильтры, тема (паритет с app.py)` — `639e617`
4. `feat(web): shared-secret гейт доступа` + тесты — `8ddaa6a`
5. Деплой-фиксы, обнаруженные только живым деплоем — `dd90352`, `55b2c44` (не были в исходном плане, см. ниже)
6. `docs: docs/backlog/BACKLOG.md/CLAUDE.md/cli_reference.md — Vercel-проект, blob-токен, команда деплоя` — closure-коммит
...
- [x] `cd web && vercel link` (подхватывает существующий `ai-policy-jobs`, новый проект не создавать) + `vercel deploy --prod` — реальный код поверх заглушки
- [x] живой смок: открыть URL, проверить фильтры/статус-запись/тему в браузере
- [x] `docs/backlog/BACKLOG.md`, `CLAUDE.md`, `docs/user_guides/cli_reference.md` обновлены

## Что разошлось с планом

Почти всё нетривиальное в этом заходе всплыло только на живом деплое — ни один из пунктов ниже не был и не мог быть пойман локальными тестами.

- **`sys.exit(main(...))` в псевдокоде §2 — баг.** Как написано, `sys.exit()` завершил бы процесс ДО строки `blob_sync.upload(...)` — публикация результата прогона никогда бы не выполнилась. Исправлено при реализации: код выхода сохраняется в переменную, `upload()` вызывается, `sys.exit(exit_code)` — последней строкой.
- **`--add-random-suffix`/`--allow-overwrite` в CLI — presence-only флаги, не boolean-value.** Первая живая попытка (`--add-random-suffix false`) включила суффикс вместо того, чтобы выключить — сравнение с `docs.vercel.com/docs/cli/blob` показало, что оба флага нужно передавать голыми (наличие = true) или не передавать вовсе (по умолчанию false). Первые два тестовых блоба с случайным суффиксом удалены вручную.
- **`web/api/_logic.py` НЕ импортирует `scripts/store.py`, вопреки §3.** Vercel-документация по Root Directory прямо говорит: «your app will not be able to access files outside of that directory» — как только `web/` становится Root Directory проекта (а он им и должен стать), обращение к `../scripts/store.py` физически невозможно. `_logic.py` несёт свою копию `STATUS_VALUES` и двух SQL-запросов — не копия схемы/миграций/upsert-логики `store.py`, ничего из этого read+status-write GUI не нужно.
- **`web/api/_blob.py` — сырой HTTP, а не CLI, вопреки отвергнутой альтернативе в §Design rationale.** Тот пункт был верен для `blob_sync.py` (локальная машина, CLI и так нужен для деплоя) — но `vercel` CLI физически недоступен ВНУТРИ задеплоенной Python-функции. Официального Python SDK для Blob нет (только неофициальный пакет одного мейнтейнера — сознательно не взят как зависимость для кода, трогающего эти данные). Контракт восстановлен из исходников `@vercel/blob` (github.com/vercel/storage) и живьём проверен против настоящего стора.
- **Три бага, найденные только реальным деплоем на Preview (`dd90352`):**
  1. `ModuleNotFoundError: No module named '_auth'` — Vercel грузит каждый `api/*.py` как изолированный entrypoint (`importlib spec_from_file_location` на этом одном файле) и НЕ добавляет его собственную директорию в `sys.path`. Тот же паттерн, что уже в `scripts/run.py`/`app/app.py`, пришлось продублировать в каждом entrypoint-файле `web/api/`.
  2. В `web/` не было Python-манифеста — сборка молча создавала пустой `pyproject.toml`, `requests` не ставился. Добавлен `web/requirements.txt`.
  3. Каждый `/api/status` падал 409/412 — не гонка, а системный баг: GET-ответы для `jobs.db` приходят со слабым ETag (`W/"..."`), а `If-Match` требует строгого сравнения (RFC 7232) и никогда не принимает слабый валидатор. Исправлено в `_blob.download_with_etag()`.
- **Read-after-write задержка в Vercel Blob's CDN — `55b2c44`.** Живьём подтверждено: GET сразу после успешного POST/status ещё около минуты отдавал старое значение. Не баг записи (сама запись всегда была надёжной и подтверждалась прямым запросом чуть позже) — баг в дизайне `updateStatus()`, который перезапрашивал состояние сразу после своей же записи. Убран лишний re-fetch: успешная запись обновляет состояние в памяти напрямую.
- **Root Directory проекта НЕ пришлось менять.** Осталось `.` на всём протяжении — `vercel deploy`, запущенный из `web/` после `vercel link` внутри неё, использует эту директорию как источник деплоя независимо от дашборд-настройки (та управляет в первую очередь git-триггерными автодеплоями). Предположение в §3 о необходимости менять Root Directory не подтвердилось.
