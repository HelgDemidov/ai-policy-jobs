...
- Новый проект в аккаунте куратора, регион **`aws-us-east-1`** (не `eu-central-1`, где лежит существующий проект «Scopus Search») — совпадает с дефолтным регионом функций Vercel (`iad1`, подтверждено заголовком `x-vercel-id: fra1::iad1::...` на живом проде; в `web/` нет `vercel.json`). Это решает именно ту задержку, ради которой затевался весь редизайн — регион БД важнее совпадения с другим, не связанным проектом.
...
- [ ] `web/api/_schema.py` — `postings`/`organizations`/`searches` Table-определения, индексы
