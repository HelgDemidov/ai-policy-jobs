...
Статус: реализовано (`edd3395`..`87457e7`) · 2026-08-04
...
- Новый проект в аккаунте куратора (`quiet-sea-26110140`, создан 2026-08-04) — **фактический регион `aws-us-west-2`, не запланированный `aws-us-east-1`**. Причина расхождения: единственный доступный инструмент провижининга в этой сессии (`mcp__neon__create_project`) не принимает параметр региона — ни через MCP, ни через `neonctl` (CLI требует интерактивный OAuth-логин в браузере, недоступный в этой среде).
- **Обновление 2026-08-04, тем же днём:** куратор указал на более глубокий вопрос — сама привязка к региону США была унаследованным дефолтом Vercel (`iad1` — «for all new projects», по документации Vercel, независимо от реальной локации пользователя), а не осознанным выбором, при этом куратор физически в Черногории и `Scopus Search` уже лежит в `eu-central-1`. Проверено: Vercel Hobby с недавних пор умеет задавать один кастомный регион функции (раньше только `iad1`). Куратор создал новый проект `divine-shape-98570790` в `aws-eu-central-1` вручную через консоль Neon (единственный способ выбрать регион), данные перенесены реальным прогоном `scripts/run.py` (не дамп/рестор — Postgres здесь зеркало, не источник правды); добавлен `web/vercel.json` (`"regions": ["fra1"]`), задеплоено вместе со сменой `DATABASE_URL` одним деплоем, чтобы не проходить через худшее промежуточное состояние. Живой замер после: `x-vercel-id: fra1::fra1::...` (было `fra1::iad1`), сквозная задержка ~0.4-0.5с в тёплом состоянии (было ~1.2-1.5с). Коммит `eab1d70`. Старый проект `quiet-sea-26110140` (`us-west-2`) оставлен нетронутым как путь отката.
...
- [x] Neon-проект создан — изначально `quiet-sea-26110140` в `aws-us-west-2` (инструмент провижининга региона не принимает); в тот же день перепровизован куратором вручную в `aws-eu-central-1` (`divine-shape-98570790`) вместе с переносом Vercel-функции на `fra1`, см. §2/«Что разошлось с планом». `DATABASE_URL` в Vercel Production+Preview и локальном `.env` — актуален на `eu-central-1`
...
- [x] `_blob.py`, `blob_sync.py`, `_logic.py`, их тесты, Blob env vars — удалены (`BLOB_WEBHOOK_PUBLIC_KEY`/`BLOB_STORE_ID`/`BLOB_READ_WRITE_TOKEN` из Vercel Production+Preview и локального `.env`; `requests` заодно убран из `web/requirements.txt` — был нужен только `_blob.py`). Сам Blob-стор в дашборде — нетронут, как решил куратор
- [x] Живая проверка на проде: фильтр (348/348 верно), пагинация «Load more» (60→120), полнотекстовый поиск (título-match первым в выдаче), статус-запись (round-trip подтверждён, без alert/отката) — все живьём через настоящий браузер. Замер задержки: EXPLAIN ANALYZE на Neon — 0.392мс чистого времени запроса; сквозная задержка клиент→прод — 0.5-1.5с, полностью объясняется несовпадением региона (см. ниже), не запросом
- [x] `docs/backlog/BACKLOG.md`, `CLAUDE.md`, `docs/user_guides/cli_reference.md` обновлены
...
- **Регион Neon: изначально `aws-us-west-2`, не запланированный `aws-us-east-1` — впоследствии исправлено на `aws-eu-central-1`.** Единственный доступный инструмент провижининга (`mcp__neon__create_project`) не принимает параметр региона; `neonctl` CLI требует интерактивный OAuth-логин в браузере, недоступный в этой среде. Первое измерение (`us-west-2`): `EXPLAIN ANALYZE` — 0.392мс чистого времени запроса, вся сквозная задержка (0.5-1.5с) — сетевой путь между `iad1` и `us-west-2`, больше, чем изначально предполагалось в спеке («60-80мс, не критично» — оценка оказалась неверной). Куратор в тот же день указал на более глубокий вопрос: сама привязка к региону США была унаследованным дефолтом Vercel, не осознанным выбором — куратор физически в Черногории, `Scopus Search` уже в `eu-central-1`. Vercel Hobby, как выяснилось, теперь позволяет задать один кастомный регион функции. Итог: новый Neon-проект в `eu-central-1` + `web/vercel.json` (`fra1`) для функции, оба переключены одним деплоем (коммит `eab1d70`). После: `x-vercel-id: fra1::fra1` (было `fra1::iad1`), сквозная задержка ~0.4-0.5с в тёплом состоянии — заметно лучше промежуточного `us-west-2`-состояния, не только исходной проблемы.
- **Две миграции вместо пяти** (см. §4) — обнаружено при реализации, что дробление на «create postings / create organizations / create searches / add org_id» симулировало бы органическую историю, которой не было: все три таблицы деплоятся в одном релизе.
- **Найден и исправлен реальный баг в собственном плане `/feature-workflow`, до написания кода**: исходный порядок коммитов ставил зависимости (`sqlalchemy`/`psycopg`/`alembic`) ПОСЛЕ модулей, которые их импортируют (`_schema.py`, Alembic, `_repo.py`) — упало бы на первом же коммите. Переставлено до начала реализации.
- **`dict(conn.execute(select(a, b)))` не работает напрямую** — `CursorResult` имеет собственный `.keys()` (имена колонок), из-за чего `dict()` ошибочно трактует результат как уже словарь-подобный вместо итерируемого набора пар. Нужен `.all()` перед `dict()`. Найдено тестами `test_postgres_sync.py`, не предусмотрено планом.
- **Защита `mirror_to_postgres` от опустошения при пустой локальной SQLite** — не было в исходном плане; добавлено по аналогии с уже существующей в `store.py` защитой от «успешный-но-пустой ответ = всё закрыто», после того как стало ясно, что у нового синк-шага нет аналогичной защиты по умолчанию.
- **`/api/facets` — целый новый эндпойнт, не было в спеке вообще.** Найдено на живой проверке (Step 6): `app.js`'s `loadFacets()` (страница `size=2000`) и `postings.py`'s `MAX_SIZE=200` — два независимо захардкоженных «большое число» без единого источника правды — разошлись, из-за чего часть организаций пропадала из списка фильтров («Showing 60 of 243» вместо настоящих 348). Куратор на месте указал на сам факт хардкода как на неправильный подход к проблеме в принципе — race conditions между независимыми константами, а не выбор конкретного числа. Правильное решение: не «фильтровать по большому размеру страницы», а отдельный `DISTINCT`-запрос (`_repo.get_facets`), не имеющий потолка по количеству строк в принципе. Коммит `87457e7`, вне исходного плана коммитов, но до закрытия спека.
- **`_blob.py`'s ETag-логика ушла полностью** — Postgres даёт транзакции из коробки, конкурентная запись `run.py`/веб-GUI решается на уровне БД, не нужен `If-Match` вручную. Предполагалось в rationale, подтвердилось на практике без дополнительных сложностей.
