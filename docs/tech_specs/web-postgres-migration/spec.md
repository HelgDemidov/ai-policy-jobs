...
- [x] Neon-проект создан (`quiet-sea-26110140`) — **фактически в `aws-us-west-2`, не в запланированном `aws-us-east-1`** (инструмент провижининга региона не принимает, см. §2/«Что разошлось с планом»); `DATABASE_URL` в Vercel Production+Preview и локальном `.env`
...
- **Регион Neon: `aws-us-west-2`, не запланированный `aws-us-east-1`.** Единственный доступный инструмент провижининга (`mcp__neon__create_project`) не принимает параметр региона; `neonctl` CLI требует интерактивный OAuth-логин в браузере, недоступный в этой среде. Последствие измерено, не предположено: `EXPLAIN ANALYZE` на реальном проде показывает 0.392мс чистого времени выполнения запроса — вся сквозная задержка (0.5-1.5с по curl+TTFB с настоящего клиента) объясняется сетевым путём между `iad1` (регион функций Vercel) и `us-west-2`, а не запросом к БД. Это больше, чем изначально предполагалось в спеке (там оценка была «60-80мс, не критично») — оценка оказалась неверной, реальная стоимость несовпадения региона выше. Тем не менее и это на порядок(и) лучше исходной проблемы (2-15 секунд, вызванных скачиванием всего Blob-файла). Живая перепровизовка в `us-east-1` вручную через консоль Neon (которая даёт выбор региона) — не выполнена в рамках этого `/feature-workflow`, зафиксирована как рекомендованный, но не обязательный следующий шаг.
