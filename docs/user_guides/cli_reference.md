...
**Веб-GUI на Vercel:** https://ai-policy-jobs.vercel.app (постоянно доступен, пароль — `SITE_PASSWORD`, см. ниже раздел «Веб-GUI»). ATS/агрегаторы вызываются исходящими HTTP-запросами, входящих портов локальный инструмент не открывает. Ключи Adzuna — `.env` в корне (не в git).
...
**Staging-ветка (`br-green-unit-as6us8ck`, `STAGING_DATABASE_URL` в `.env`)** — copy-on-write форк `production`, полная копия боевых данных без отдельного заполнения. Использовать для любой ручной живой проверки (curl/браузер/SQL напрямую) вместо прода — не тот же случай, что CI-джоб `test-integration` (тот проверяет код на чистой БД), это для «пощупать руками реальные данные, не рискуя ими». Не обновляется автоматически при изменении прода — при необходимости пересоздать (`create_branch` заново от `production`) или `reset_from_parent`, если понадобится актуализировать.
...
- `config/searches.yaml` — поисковые запросы: `{id, source: adzuna|jobspy_linkedin|jobspy_indeed, query/phrase, country/location, manual: true (опц.)}`.
