...
- **`orgs.yaml` + `scripts/`** — рабочий инструмент мониторинга вакансий: коннекторы к ATS (Lever/Greenhouse/Personio) → локальный SQLite (`data/jobs.db`, gitignored). Запуск: `python3 scripts/run.py`.
