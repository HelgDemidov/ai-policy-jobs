...
- **`.github/`** — CI (`workflows/tests.yml`, runs `ruff check` + `tests/` on push/PR to `master`) + `dependabot.yml` (pip + npm + github-actions, weekly). No branch protection — GitHub Pro is required for that on a private personal-account repo.
